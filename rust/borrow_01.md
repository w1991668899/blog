...
通常在不同的使用场景中，rust会为每一个类型提供不同的表示形式。对值的存储地址的管理可以通过以下方式根据特定用途选择合适的
指针类型，例如 [`Box<T>`] 、[`Rc<T>`]。除了这些可用于任何类型的通用包裹器外，有些类型提供了可选择的高成本功能。例如 
[`string`]，它增加了将string转为str的能力，这就需要为一个简单的不可变的字符串存储不必要的附加信息。

这些类型通过引用底层数据来提供对数据的访问。通常称为对该类型的借用。例如 [`Box<T>`] 是对T的借用， [`String`] 是对str的借用。

类型可以被借用是通过实现 `Borrow<T>` 来表示，trait的方法[`borrow`]提供了对`T`的引用。一个类型可以被借用为多种不同类型。如果
希望对类型实现可变借用，可以实现[`BorrowMut<T>]
